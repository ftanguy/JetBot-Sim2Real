...
class LogWrapper(GymnaxWrapper):
    ...
    def step(self, key: chex.PRNGKey, state: LogEnvState, action: Union[int, float], params: Optional[environment.EnvParams] = None) -> Tuple[chex.Array, environment.EnvState, float, float, bool, dict]:
        # Extract safety_indicator from the inner env_state and add it to the info dict 
        # to support Lagrangian constraint modes without breaking the wrapper abstraction.
        ...
        # Expose safety indicator to the training loop
...
class FrameStackWrapper(GymnaxWrapper):
    """
    Stacks consecutive observations along the feature dimension to provide finite memory.
    Note: Typically not used when a recurrent policy (LSTM) is active.
    """
    def __init__(self, env, num_stack=3):
        super().__init__(env)
        self.num_stack = num_stack
        
        # Calculate the expanded observation shape (e.g., 10 features * 3 frames = 30)
    ...
    def observation_space(self, params):
        # Expand the observation space bounds to match the stacked shape
    ...
    def reset(self, key, params=None):
        ...
        # Create initial stack [obs, obs, obs] because there is no history yet
    ...
    def step(self, key, state, action, params=None):
        ...
        # Bypass array concatenation if no stacking is applied (num_stack == 1)
        if self.num_stack == 1:
            new_stack = obs
        else:
            # Shift the stack: drop the oldest observation, append the newest one at the end
