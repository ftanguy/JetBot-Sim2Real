...
# Load empirical physics lookup table at startup
try:
    PHYSICS_TABLE = jnp.load("physics_lookup.npy")
    PHYSICS_TABLE = jnp.array(PHYSICS_TABLE)
    print("Loaded Data-Driven Physics Table.")
except:
    print("ERROR: physics_lookup.npy not found. Using dummy.")
    PHYSICS_TABLE = jnp.zeros((41, 2000))
...
class JAXJetBotEnvReplicated(environment.Environment):
    ...
    Utilizes a Data-Driven empirical lookup table for accurate low-friction physics.
    """
    ...
    def deadzone_response(self, u, d_minus, d_plus, alpha_minus, alpha_plus):
        """Mathematical deadzone fallback (Replaced by Data-Driven lookup in this phase)."""
    ...
    def _get_best_goal_idx(self, pos, last_goal_idx=None):
        ...
        # Hysteresis Logic to prevent rapid target switching
    ...
    def step_env(
        ...
        # --- Stiction Breaking ("Dance") Logic ---
        # Calculate spatial wiggle (turning intensity) to break static friction
        turn_intensity = jnp.abs(u_action[0] - u_action[1])
        WIGGLE_GAIN = 0.15 
        
        # Add boost in the direction of the intended movement
        boost = turn_intensity * WIGGLE_GAIN
        u_left_effective = u_action[0] + jnp.sign(u_action[0]) * boost
        u_right_effective = u_action[1] + jnp.sign(u_action[1]) * boost
        
        # --- Data Driven Physics Execution ---
        key, k1, k2 = jax.random.split(key, 3)
        w_left = self._sample_physics(k1, u_left_effective)
        w_right = self._sample_physics(k2, u_right_effective)
    ...
    def _sample_physics(self, rng, u_val):
        """Samples actual speeds from empirical lab data lookup table."""
        u_val = jnp.clip(u_val, -1.0, 0.999)
        bin_idx = jnp.floor((u_val + 1.0) / 0.05).astype(int)
        sample_idx = jax.random.randint(rng, shape=(), minval=0, maxval=2000)
        speed = PHYSICS_TABLE[bin_idx, sample_idx]
        return speed
