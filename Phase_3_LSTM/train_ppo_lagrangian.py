...
class ScannedRNN(nn.Module):
    """LSTM sequence processor for Recurrent PPO."""
    @nn.compact
    def __call__(self, carry, x):
        rnn_state, x = nn.LSTMCell(features=128)(carry, x)
        return rnn_state, x

class ActorCritic(nn.Module):
    """
    Recurrent Actor-Critic network. 
    Uses an LSTM cell to build a hidden memory state across timesteps.
    """
    ...
    def __call__(self, hidden, x):
        ...
        # 1. Embedding Layer (Process input before memory)
        embedding = nn.Dense(128, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))(x)
        embedding = activation(embedding)

        # 2. LSTM Layer
        hidden, x_rnn = ScannedRNN()(hidden, embedding)

        # --- Actor ---
        actor_mean = nn.Dense(128, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))(x_rnn)
        ...
        # --- Critic ---
        critic = nn.Dense(128, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))(x_rnn)
    ...
    def initialize_carry(batch_size):
        """Initializes the LSTM cell and hidden states."""
        return (jnp.zeros((batch_size, 128)), jnp.zeros((batch_size, 128)))
...
def make_train(config):
    ...
    def train(rng):
        ...
        init_hidden = network.initialize_carry(1) 
        network_params = network.init(_rng, init_hidden, init_x)["params"]
        ...
        train_state_hidden = network.initialize_carry(config["NUM_ENVS"])

        @jax.jit
        def _primal_update_step(runner_state, update_count):
            train_state, env_state, last_obs, last_done, hidden_state, rng, lagrangian_state = runner_state

            # --- Rollout Phase ---
            def _env_step(runner_state, step_idx):
                train_state, env_state, last_obs, last_done, hidden_state, rng = runner_state

                # Reset hidden state if episode ended last step
                hidden_state = jax.tree_map(
                    lambda x: x * (1 - last_done)[:, None], 
                    hidden_state
                )

                rng, _rng = jax.random.split(rng)
                new_hidden_state, pi, value = network.apply({"params": train_state.params}, hidden_state, last_obs)
                ...
                transition = Transition(
                    done, action, value, reward, cost, log_prob, last_obs, info, 
                    action_mean, action_stddev, hidden_state
                )

                return (train_state, env_state, obsv, done, new_hidden_state, rng), transition

            rollout_runner_state = (train_state, env_state, last_obs, last_done, hidden_state, rng)
            rollout_runner_state, traj_batch = jax.lax.scan(_env_step, rollout_runner_state, jnp.arange(config["NUM_STEPS"]))
            
            train_state, env_state, last_obs, last_done, hidden_state, rng = rollout_runner_state

            # --- Advantage Calculation Phase ---
            _, _, last_val = network.apply({"params": train_state.params}, hidden_state, last_obs)
            ...
            def _update_epoch(update_state, epoch_num): 
                ...
                # Time Sequence Preservation: Swap axes to (Num_Envs, Num_Steps, ...)
                batch = jax.tree_map(lambda x: jnp.swapaxes(x, 0, 1), 
                                     (traj_batch, advantages, targets))
                
                env_perm = jax.random.permutation(_rng, config["NUM_ENVS"])
                batch = jax.tree_map(lambda x: x[env_perm], batch)
                ...
                def _loss_fn(params, traj, gae, targ):
                    ...
                    # Extract initial hidden state for each sequence in the batch
                    init_h = jax.tree_map(lambda x: x[:, 0, :], traj.hidden_state)
                    
                    def _scan_lstm(carry, x):
                        hidden, done_mask = carry
                        obs = x
                        new_hidden, pi, value = network.apply({"params": params}, hidden, obs)
                        return (new_hidden, None), (pi.loc, pi.scale_diag, value)

                    _, (loc_seq, scale_seq, value_seq) = jax.lax.scan(
                        _scan_lstm, 
                        (init_h, None), 
                        jnp.swapaxes(traj.obs, 0, 1)
                    )

                    pi_seq = distrax.MultivariateNormalDiag(loc_seq, scale_seq)

                    actions_time_major = jnp.swapaxes(traj.action, 0, 1)
                    log_prob_time_major = pi_seq.log_prob(actions_time_major)
                    value_predicted_time_major = value_seq
                    
                    log_prob = jnp.swapaxes(log_prob_time_major, 0, 1)
                    value_predicted = jnp.swapaxes(value_predicted_time_major, 0, 1)
                    
                    log_prob = log_prob.reshape(-1)
                    value_predicted = value_predicted.reshape(-1)
                    
                    targ_flat = targ.reshape(-1)
                    gae_flat = gae.reshape(-1)
                    traj_log_prob_flat = traj.log_prob.reshape(-1)
                    traj_value_flat = traj.value.reshape(-1)

                    if config["CLIP_VF"]:
                        value_predicted = traj_value_flat + (value_predicted - traj_value_flat).clip(-config["CLIP_EPS"], config["CLIP_EPS"])
                    value_loss = 0.5 * jnp.square(targ_flat - value_predicted).mean()

                    ratio = jnp.exp(log_prob - traj_log_prob_flat)
                    ...
                    return total_loss, {
                        ...
                        "approx_kl": approx_kl, "value_minus_targ_mean": jnp.mean(value_predicted - targ_flat),
                        "value_mean": jnp.mean(value_predicted), "targ_mean": jnp.mean(targ_flat),
            ...
            return (train_state, env_state, last_obs, last_done, hidden_state, rng, lagrangian_state), traj_batch

        last_done = jnp.zeros(config["NUM_ENVS"], dtype=bool)
        runner_state = (train_state, env_state, obsv, last_done, train_state_hidden, rng, lagrangian_state)
        all_metrics = [] 
        ...
        for update_step in tqdm(range(config["TOTAL_PPO_UPDATES"]), desc="PPO Updates"):
            ...
            if (update_step + 1) % config["H_CHUNK_SIZE"] == 0:
                ...
                runner_state = (train_state, env_state, last_obs, last_done, hidden_state, rng, lagrangian_state)
