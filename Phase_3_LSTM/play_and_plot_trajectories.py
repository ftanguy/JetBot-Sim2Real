...
def plot_maze(ax, maze_config_path="maze_cfg.yaml", env_params=None):
    ...
    ax.set_title("Trajectories at Test Time (LSTM)")
    ax.set_xlabel("X coordinate (m)")
    ax.set_ylabel("Y coordinate (m)")
    ax.legend()

def run_evaluation(trained_params, config, num_episodes=100):
    """Runs a batch of evaluation episodes utilizing the trained LSTM policy."""
    ...
    def run_one_episode(rng_key):
        # Initialize LSTM hidden state for a single unbatched episode execution.
        # We initialize with batch=1, then squeeze to (128,) to force unbatched mode.
        hidden = network.initialize_carry(1)
        hidden = jax.tree_map(lambda x: x.squeeze(0), hidden)

        def _step_fn(carry, unused):
            obs, state, rng_key, hidden = carry
            
            # Unbatched network inference
            new_hidden, pi, _ = network.apply({"params": trained_params}, hidden, obs)
            action = pi.mode()
            
            rng_key, step_key = jax.random.split(rng_key)

            new_obs, new_state, _, _, done, _ = env.step(step_key, state, action, env_params)
            real_pos = new_state.env_state.pos
            
            return (new_obs, new_state, rng_key, new_hidden), (real_pos, done)

        rng_key, reset_key = jax.random.split(rng_key)
        obs, state = env.reset(reset_key, env_params)
        
        initial_carry = (obs, state, rng_key, hidden)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Initialize unbatched dummy hidden state for parameter restoration
        init_hidden = network.initialize_carry(1)
        init_hidden = jax.tree_map(lambda x: x.squeeze(0), init_hidden)
        
        empty_params = network.init(rng, init_hidden, init_x)["params"]
