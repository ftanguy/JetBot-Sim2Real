...
class EnvState:
    ...
    # Physics parameters specific to this environment instance
    alpha: float
    d_plus: float
    d_minus: float
...
class JAXJetBotEnvReplicated(environment.Environment):
    ...
    # --- HELPER: SMART GOAL SELECTION WITH HYSTERESIS ---
    def _get_best_goal_idx(self, pos, last_goal_idx=None):
        ...
        # PENALTY: 1.5 meters (Balanced detour length)
        LOS_PENALTY = 0.75
        scores = all_dists + (1.0 - all_visible.astype(jnp.float32)) * LOS_PENALTY

        # --- HYSTERESIS LOGIC ---
        # If we have a previous target, give it a "bonus" (reduce its score/cost)
        # to encourage the robot to stick with it unless another goal is MUCH better.
        HYSTERESIS_BONUS = 0.1 # meters
        
        # Modify the score only if last_goal_idx is provided (not None)
        if last_goal_idx is not None:
             # Create a one-hot mask for the last goal
             mask = (jnp.arange(self.goals.shape[0]) == last_goal_idx).astype(jnp.float32)
             
             # Subtract bonus from the specific goal's score
    ...
    def step_env(
        ...
        # Identify the current best goal, applying Hysteresis using last_goal_idx
        ...
        distance_progress = jax.lax.select(
            goal_has_switched,
            0.0, # Zero reward on switch
            raw_progress # Allow negative reward on backup
        )
        ...
        # 1. Soft Limit Penalty (Overpower)
        # "Free" zone: [-0.2, 0.2]
        # "Taxed" zone: > 0.2
        excess_u = jnp.maximum(0.0, jnp.abs(raw_action) - 0.2)
        
        # Strong Coefficient: -3.0
        # Forces the agent to evaluate if excessive speed is worth the penalty
        overpower_penalty = -3.0 * jnp.sum(jnp.square(excess_u))

        # 2. Centering Penalty
        centering_penalty = -0.1 * jnp.sum(jnp.square(raw_action)) 
        # --------------------------------------
        ...
        new_state = EnvState(
            ...
            time=state.time + 1,
            last_goal_idx=best_idx, # Update the sticky goal
            alpha=state.alpha,
            d_plus=state.d_plus,
    ...
    def reset_env(self, key: chex.PRNGKey, params: EnvParams) -> Tuple[chex.Array, EnvState]:
        key, pos_key, theta_key, alpha_key, d_plus_key, d_minus_key = jax.random.split(key, 6)

        sampled_alpha = jax.random.uniform(alpha_key, minval=params.deadzone_alpha_min, maxval=params.deadzone_alpha_max)
        sampled_d_plus = jax.random.uniform(d_plus_key, minval=params.deadzone_d_plus_min, maxval=params.deadzone_d_plus_max)
        sampled_d_minus = jax.random.uniform(d_minus_key, minval=params.deadzone_d_minus_min, maxval=params.deadzone_d_minus_max)

        pos = self._sample_safe_position(pos_key, params)
        theta = jax.random.uniform(theta_key, minval=-jnp.pi, maxval=jnp.pi)
        
        # Initial Selection: No Hysteresis (pass None)
        ...
        state = EnvState(
            ...
            alpha=sampled_alpha,
            d_plus=sampled_d_plus,
            d_minus=sampled_d_minus
        )
    ...
    def _get_distance_and_angle_to_specific_goal(selfself, robot_pos, robot_yaw, goal_idx):
    ...
    def _compute_cost(self, dist_to_obs: chex.Array, params: EnvParams) -> float:
        ...
        # Normalized distance: 0.0 (Safe Limit) -> 1.0 (Touching Wall)
        norm_dist = jnp.maximum(0, 1 - dist_to_obs / min_safe_dist)
        
        # Exponential Curve
        # k=10.0 creates a sharp rise only when very close
        k = 10.0
